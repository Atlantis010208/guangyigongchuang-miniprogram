  ...
  async onSubmitOrder(){
    ...
    try{
      ...
      if (db) {
        ...
        try{
          const r1 = await util.callCf('requests_create', { request: { orderNo: id, category: 'hotel', params: order.params, userId, status: 'submitted' } })
          ...
          if (msg.indexOf('collection not exists') !== -1 || (err && err.errCode === -502005)) {
            if (wx.cloud && wx.cloud.callFunction) {
              ...
              await util.callCf('requests_create', { request: { orderNo: id, category: 'hotel', params: order.params, userId, status: 'submitted' } }).catch(()=>{})
            }
          }
        }
        util.callCf('orders_create', { order: { type:'products', orderNo: id, category:'hotel', params: order.params, status:'submitted', paid:false, userId } }).catch(()=>{})
      }
    }catch(err){}
