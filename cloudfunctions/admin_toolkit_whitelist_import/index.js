/**
 * 云函数：admin_toolkit_whitelist_import
 * 功能：批量导入工具包白名单数据
 * 权限：仅管理员
 * 
 * 入参：
...
exports.main = async (event) => {
  ...
  try {
    ...
    try {
      ...
      console.error('[admin_toolkit_whitelist_import] 文件解析失败:', parseErr)
      return {
        success: false,
        code: 'PARSE_ERROR',
    ...
    for (let i = 0; i < newPhones.length; i += batchSize) {
      ...
      try {
        ...
        for (const doc of docs) {
          try {
            await db.collection('toolkit_whitelist').add({ data: doc })
            successCount++
          } catch (insertErr) {
            // 唯一索引冲突，跳过
            if (insertErr.errCode === -502001 || (insertErr.message && insertErr.message.includes('duplicate'))) {
              console.log('[admin_toolkit_whitelist_import] 重复记录跳过:', doc.phone)
        ...
        console.error('[admin_toolkit_whitelist_import] 批量插入失败:', batchErr)
      }
    }
    
    console.log('[admin_toolkit_whitelist_import] 导入完成，成功:', successCount)
    ...
    console.error('[admin_toolkit_whitelist_import] Error:', err)
    return {
      success: false,
      code: 'SERVER_ERROR',
