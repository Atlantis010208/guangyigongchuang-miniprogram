...
exports.main = async (event) => {
  try {
    ...
    let res
    if (id) {
      res = await col.doc(id).update({ data: { ...patch, updatedAt: Date.now() } })
    } else {
      res = await col.where({ orderNo }).update({ data: { ...patch, updatedAt: Date.now() } })
    }
    return { success: true, code: 'OK', data: { updated: res && res.stats ? res.stats.updated : 0 } }
  } catch (err) {
    return { success: false, code: 'REQUESTS_UPDATE_FAILED', errorMessage: err && err.message ? err.message : 'unknown error' }
  }
}
