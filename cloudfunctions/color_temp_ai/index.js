...
- 如果客户选了两种灯具，layers 必须包含对应的两条建议
- 如果只有一种灯具，layers 只返回一条
- tips 要有针对性，结合空间类型和人群特点
- 只输出 JSON，不要输出任何其他内容`

// 构造专业灯光设计师 Prompt（优先从数据库读取，降级使用默认模板）
async function buildPrompt(params) {
  const { space, area, targetLux, age, usage, primaryFixture, secondaryFixture } = params
  ...
  return template
    ...
    .replace(/\$\{area\}/g, area ? area + '㎡' : '未提供')
    .replace(/\$\{targetLux\}/g, targetLux ? targetLux + 'Lx' : '未提供')
    .replace(/\$\{age\}/g, age || '未指定')
    .replace(/\$\{usage\}/g, usage || '日常起居')
    .replace(/\$\{primaryFixture\}/g, primaryFixture || '吸顶灯/筒灯')
    .replace(/\$\{secondaryFixture\}/g, secondaryFixture || '无')
}
...
exports.main = async (event) => {
  try {
    ...
    const prompt = await buildPrompt({
      ...
      age: ageName,
      usage: usageName,
      primaryFixture: primaryFixtureName,
      secondaryFixture: secondaryFixtureName
    })
