/**
 * 云函数：admin_whitelist_list
 * 功能：分页查询课程白名单，支持筛选和搜索
 * 权限：仅管理员
 * 
 * 入参：
 *   - limit: 每页数量，默认 20
...
exports.main = async (event) => {
  ...
  try {
    ...
    const [totalRes, pendingRes, activatedRes] = await Promise.all([
      db.collection('course_whitelist').count(),
      db.collection('course_whitelist').where({ status: 'pending' }).count(),
      db.collection('course_whitelist').where({ status: 'activated' }).count()
    ])
    ...
    // 4. 查询符合条件的总数
    const countRes = await db.collection('course_whitelist')
      .where(where)
      .count()
    const filteredTotal = countRes.total || 0
    
    // 5. 分页查询数据
    const listRes = await db.collection('course_whitelist')
      .where(where)
    ...
    const data = listRes.data.map(item => ({
      ...
      courseName: item.courseName || '灯光设计课',
      status: item.status,
    ...
    console.error('[admin_whitelist_list] Error:', err)
    return {
      success: false,
      code: 'SERVER_ERROR',
